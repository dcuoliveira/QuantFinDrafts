...
instruments = ['USDBRL', 'USDCLP', 'USDZAR', 'USDMXN', "USDCOP",
               ...
               "USDINR", "USDKRW", "USDTWD", "USDCNH"]

file = open(os.path.join(os.getcwd(), "src", "data", "fxmm-1.pickle"), 'rb')
...
for inst in instruments:
    ...
    signals.append(pd.DataFrame(tmp_signals.mean(axis=1), columns=["{} signals".format(inst)]))
    forecasts.append(tmp_forecasts)

bars_df = pd.concat(bars, axis=1).resample("B").last()
vols_df = pd.concat(vols, axis=1).resample("B").last()
rets_df = pd.concat(rets, axis=1).resample("B").last()
signals_df = pd.concat(signals, axis=1).resample("B").last()
forecasts_df = pd.concat(forecasts, axis=1).resample("B").last()
carrys_df = pd.concat(carrys, axis=1).resample("B").last()

rank_df = signals_df.rank(axis=1)
low_quantile = rank_df.quantile(q=0.2, axis=1)
high_quantile = rank_df.quantile(q=0.8, axis=1)
...
forecastsx_df = high_df.add(low_df, axis=1)
forecastsx_df = forecastsx_df[["{} signals".format(inst) for inst in instruments]]
forecastsx_df.columns = forecasts_df.columns
forecasts_df = forecastsx_df.copy()
...
for t in tqdm(backtest_dates, desc="Running Backtest", total=len(backtest_dates)):
    ...
    for inst in valid_instruments:
        ...
        if first_day:
            ...
            price_change = bars_df.loc[t, "{} close".format(inst)] - bars_df.loc[t - BDay(1), "{} close".format(inst)]
            ...
            forecast = forecastsx_df.loc[t, "{} forecasts".format(inst)]
    ...
    if not first_day:
        ...
        for inst in valid_instruments:
            previous_price = bars_df.loc[t - BDay(1), "{} close".format(inst)]
        ...
        for inst in instruments:
            ...
            if (previos_positions_units != 0) and (not pd.isna(previos_positions_units)):
                ...
                pnl += inst_pnl
                nominal_ret += portfolio_df.loc[t - BDay(1), "{} w".format(inst)] * rets_df.loc[t, "{} ret%".format(inst)]
